...
import { ALWAYS_EXCLUDED_DIRS, FALLBACK_EXCLUDED_DIRS, dirnameOf, isIndexable } from '@orbit-code/graph/languages';
import { extendLayout } from '@orbit-code/graph/layoutExtend';
import type { ListedFiles } from '@orbit-code/indexer/listFiles';
import type { GraphFile, LayoutSnapshot } from '@orbit-code/protocol';
import type { WatchedChanges } from './changeBatcher';
import { IndexCancelled, type IndexerProgress, runIndexer } from './indexClient';
...
export interface WorkspaceFolderInfo {
  /** Display name. */
  name: string;
  /** Absolute path: the graph's root. */
  path: string;
}

/** What GraphService needs from the editor it runs in. */
export interface GraphServiceOptions {
  /** A directory the editor keeps for this workspace; graph.json and the layouts go in orbit-v1/ inside it. */
  storageDir: string;
  /** @orbit-code/indexer's dist/indexer.mjs, run on a worker thread. */
  indexerPath: string;
  log: Logger;
  maxFiles: () => number;
  /** The folder the graph is of: the first workspace folder, or undefined with none open. */
  folder: () => WorkspaceFolderInfo | undefined;
  /** The folder's indexable files: git ls-files in a work tree, otherwise the editor's own file search. */
  listFiles: (folder: WorkspaceFolderInfo, maxFiles: number) => Promise<ListedFiles>;
}
 ...
 * from file changes, and frozen layouts. Knows nothing about panels, sessions or editors.
 */
export class GraphService implements Disposable {
  private readonly statusEmitter = new Emitter<GraphStatus>();
  private readonly graphEmitter = new Emitter<LoadedGraph>();
  ...
  private indexing: AbortController | undefined;
  ...
  constructor(private readonly options: GraphServiceOptions) {
    ...
    this.store = new Store(options.storageDir);
  }
  ...
  async load(reindex: boolean): Promise<void> {
    ...
    try {
      ...
      const cached = stored !== undefined && stored.root === folder.path;
      ...
      // Files may have changed while the editor was closed.
      if (cached) this.refresh();
    } catch (error) {
      if (error instanceof IndexCancelled || generation !== this.generation) return;
  ...
  private async index(folder: WorkspaceFolderInfo): Promise<GraphFile> {
    this.statusEmitter.fire({ phase: 'indexing', message: 'Listing files', progress: 0.02 });
    const listed = await this.options.listFiles(folder, this.options.maxFiles());
    ...
    try {
      const request = { root: folder.path, files: listed.files, truncated: listed.truncated };
    ...
    try {
      while (this.pending && this.loaded && !this.loading) {
        ...
        try {
          ...
          if (!(error instanceof IndexCancelled)) this.log.error(`graph update failed: ${error instanceof Error ? (error.stack ?? error.message) : String(error)}`);
        }
      }
    } finally {
  ...
  private async update(base: LoadedGraph, job: RefreshJob): Promise<void> {
    const folder = this.options.folder();
    if (!folder || folder.path !== base.graph.root) return;
    const generation = this.generation;
    ...
    let graph: GraphFile;
    try {
      const request = { root: folder.path, files: listed.files, truncated: listed.truncated, previous: base.graph, changed: forced };
      graph = await runIndexer(this.options.indexerPath, request, () => undefined, cancellation.signal);
    } finally {
      if (this.indexing === cancellation) this.indexing = undefined;
    }
    if (!current()) return;
