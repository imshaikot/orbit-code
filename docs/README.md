...
Orbit Code brings open-source, live 3D visuals to Claude Code, and is available for VS Code, as a desktop app and as a web client. It renders a workspace as a 3D force-directed dependency graph and animates a live Claude Code session moving through it; all three are built from the same engine and the same UI in this monorepo.

These pages are also published, with the install guide and the changelog, at [orbit-code.imshaikot.com](https://orbit-code.imshaikot.com/docs/). The site is built from this directory and the READMEs linked below, so a fix here reaches it too.
