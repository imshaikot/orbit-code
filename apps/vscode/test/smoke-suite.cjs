// Runs inside the VS Code extension host started by apps/vscode/test/smoke.mjs, on the whole workspace.
//
...
exports.run = async function run() {
  ...
  check(graph.nodes.some((node) => node.id === 'packages/webview/src/main.ts'), 'packages/webview/src/main.ts is missing from the graph');
  ...
  const smokeId = 'packages/graph/src/__orbitSmoke.ts';
  ...
  try {
    ...
    await waitFor(
      () => readGraph().edges.some((edge) => edge.source === smokeId && edge.target === 'packages/graph/src/columnar.ts'),
      60_000,
  ...
  const menuId = 'packages/graph/src/__orbitSmokeMenu.ts';
  const movedId = 'packages/graph/src/__orbitSmokeMoved.ts';
