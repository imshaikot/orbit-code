...
- **Follow.** Claude's star never shrinks out of sight, however far out you zoom, and always takes clicks. Click it for **Follow Spark**: the camera eases in to frame it, then keeps pace with it as it moves between files, while you keep full control of the camera (drag and zoom keep working, and whatever view you set is what following then keeps). Click the star again to stop.
