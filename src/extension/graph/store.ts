...
/** storageUri/orbit-v1/: graph.json plus one frozen layout per graph hash. */
export class Store {
  private readonly dir: vscode.Uri;

  constructor(context: vscode.ExtensionContext) {
    this.dir = vscode.Uri.joinPath(context.storageUri ?? context.globalStorageUri, 'orbit-v1');
  }

  async readGraph(): Promise<GraphFile | undefined> {
    try {
      const graph = JSON.parse(decoder.decode(await vscode.workspace.fs.readFile(this.file('graph.json'))));
  ...
  async writeGraph(graph: GraphFile): Promise<void> {
    ...
    await vscode.workspace.fs.writeFile(this.file('graph.json'), encoder.encode(JSON.stringify(graph)));
  }

  async readLayout(hash: string): Promise<LayoutSnapshot | undefined> {
    try {
      return decodeLayout(await vscode.workspace.fs.readFile(this.file(layoutName(hash))), hash);
    } catch {
      return undefined;
    }
  }
  ...
  async writeLayout(layout: LayoutSnapshot): Promise<void> {
    await vscode.workspace.fs.createDirectory(this.dir);
    await vscode.workspace.fs.writeFile(this.file(layoutName(layout.hash)), encodeLayout(layout));
    await this.pruneLayouts(layoutName(layout.hash));
  }

  private async pruneLayouts(keep: string): Promise<void> {
    const entries = (await vscode.workspace.fs.readDirectory(this.dir)).filter(([name]) => name.startsWith('layout-') && name !== keep);
    if (entries.length < KEEP_LAYOUTS) return;
    const stats = await Promise.all(entries.map(async ([name]) => ({ name, mtime: (await vscode.workspace.fs.stat(this.file(name))).mtime })));
    stats.sort((a, b) => b.mtime - a.mtime);
    for (const { name } of stats.slice(KEEP_LAYOUTS - 1)) await vscode.workspace.fs.delete(this.file(name));
  }

  private file(name: string): vscode.Uri {
